...
export class ReservationsService {
    ...
    async modify(id: number, userId: number, data: { startAt?: Date; endAt?: Date }) {
        const reservation = await this.prisma.reservation.findUnique({ where: { id } });
        ...
        const updatedStartAt = data.startAt ?? reservation.startAt;
        const updatedEndAt = data.endAt ?? reservation.endAt;
        const now = new Date();

        if (updatedStartAt <= now) {
            throw new BadRequestException("Start time cannot be in the past.");
        }
        if (updatedEndAt <= updatedStartAt) {
            throw new BadRequestException("End time must be after start time.");
        }

        return this.prisma.$transaction(async (prisma) => {
            const lockedRoomsRows = await prisma.$queryRaw<Array<{ id: number }>>`SELECT id FROM "Room" WHERE id = ${reservation.roomId} FOR UPDATE`;

            if (lockedRoomsRows.length === 0) {
                throw new NotFoundException(`Room with ID ${reservation.roomId} not found.`);
            }
            ...
            const overlap = await prisma.reservation.findFirst({
                where: {
                    ...
                    startAt: { lt: updatedEndAt },
                    endAt: { gt: updatedStartAt },
                },
                select: { id: true },
            })
            ...
            return prisma.reservation.update({
                ...
                data: {
                    ...
                    endAt: updatedEndAt,
                } satisfies Prisma.ReservationUpdateInput,
            });
        })
    }
}